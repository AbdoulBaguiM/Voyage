...
public class RefreshTokenService {
    @Value("${voyage.app.jwtRefreshExpirationMs}")
    ...
    public RefreshToken verifyExpiration(RefreshToken token) {
        if (token.getExpiryDate().compareTo(Instant.now()) < 0) {
            ...
            throw new TokenRefreshException(token.getToken(), "Le jeton de connexion a expiré. Veuillez vous connecter de nouveau");
        }
        return token;
    }
    @Transactional
